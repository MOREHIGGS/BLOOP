...
def loadData(scanResultsList):
    ...
    for scanResults in scanResultsList:
        ...
        for result in scanResults:
            if data["strong"] > 0.6:
                strength.append((result["strong"]))
                bmInput.append((list(result["bmInput"].values())))
    ...
    return strengthList, bmInputList, TcList, bmNumberList

# do something like filesToLoad = [dir/Result_{i} for i in range(n)]
...
# This is magic that makes plots, might rewrite later
inputData = (
    (
        Tc2Loop,
        ms12Loop,
